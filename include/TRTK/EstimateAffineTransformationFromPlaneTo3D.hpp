/*
    ...
    Version 0.1.0 (2012-05-21)
*/

/** \file EstimateAffineTransformationFromPlaneTo3D.hpp
  * \brief This file contains the \ref TRTK::EstimateAffineTransformationFromPlaneTo3D
  *        "EstimateAffineTransformationFromPlaneTo3D" class.
  */


#ifndef ESTIMATE_AFFINE_TRANSFORMATION_FROM_PLANE_TO_3D_HPP_8970243897
#define ESTIMATE_AFFINE_TRANSFORMATION_FROM_PLANE_TO_3D_HPP_8970243897
...
/** \class EstimateAffineTransformationFromPlaneTo3D
  ...
  * in terms of least squares. The source points are assumed to lie in the
  * x-y-plane. The point sets must have the same cardinality, and the points
  * must correspond to each other. There must be at least four corresponding
  * point pairs.
  *
  ...
  *     Coordinate<double> source_point3(-1,  1,  0);
  *     Coordinate<double> source_point4( 2,  0,  0);
  *
  ...
  *
  *     EstimateAffineTransformationFromPlaneTo3D<double> estimateAffineTransformationFromPlaneTo3D(source_points,
  *                                                                                                 target_points);
  *     estimateAffineTransformationFromPlaneTo3D.compute();
  *
  *     // Display the results.
  *
  *     cout.precision(4);
  ...
  *          << estimateAffineTransformationFromPlaneTo3D.getTransformationMatrix() << endl << endl;
  *
  *     // Example of how to use the result.
  *
  *     Transform3D<double> transform2 = estimateAffineTransformationFromPlaneTo3D.getTransformationMatrix();
  *
  ...
  * \endcode
  *
  * \see EstimateAffineTransformation,
...
template <class T>
class EstimateAffineTransformationFromPlaneTo3D : public EstimateTransformation3D<T>
{
private:
...
public:
    ...
    EstimateAffineTransformationFromPlaneTo3D();

    EstimateAffineTransformationFromPlaneTo3D(const std::vector<Coordinate<T> > & source_points,
                                              const std::vector<Coordinate<T> > & target_points);

    EstimateAffineTransformationFromPlaneTo3D(const std::vector<Vector3T> & source_points,
                                              const std::vector<Vector3T> & target_points);

    EstimateAffineTransformationFromPlaneTo3D(const std::vector<Vector4T, Eigen::aligned_allocator<Vector4T> > & source_points,
                                              const std::vector<Vector4T, Eigen::aligned_allocator<Vector4T> > & target_points);

    virtual ~EstimateAffineTransformationFromPlaneTo3D();
...
template <class T>
EstimateAffineTransformationFromPlaneTo3D<T>::EstimateAffineTransformationFromPlaneTo3D()
{
}
...
template <class T>
EstimateAffineTransformationFromPlaneTo3D<T>::EstimateAffineTransformationFromPlaneTo3D(const std::vector<Coordinate<T> > & source_points,
                                                                                        const std::vector<Coordinate<T> > & target_points)
{
...
template <class T>
EstimateAffineTransformationFromPlaneTo3D<T>::EstimateAffineTransformationFromPlaneTo3D(const std::vector<Vector3T> & source_points,
                                                                                        const std::vector<Vector3T> & target_points)
{
    super::setSourcePoints(source_points);
    super::setTargetPoints(target_points);
...
template <class T>
EstimateAffineTransformationFromPlaneTo3D<T>::EstimateAffineTransformationFromPlaneTo3D(const std::vector<Vector4T, Eigen::aligned_allocator<Vector4T> > & source_points,
                                                                                        const std::vector<Vector4T, Eigen::aligned_allocator<Vector4T> > & target_points)
{
    super::setSourcePoints(source_points);
    super::setTargetPoints(target_points);
...
template <class T>
void EstimateAffineTransformationFromPlaneTo3D<T>::compute()
{
    if (super::m_source_points.size() != super::m_target_points.size())
    {
        ErrorObj error;
        error.setClassName("EstimateAffineTransformationFromPlaneTo3D<T>");
    ...
    By using homogeneous coordinates the above affine transformation can be described as follows:

    | t_11  t_12  0  t_14 |     | source_x |     | target_x |
    | t_21  t_22  0  t_24 |  *  | source_y |  =  | target_y |
    | t_31  t_32  0  t_34 |     | source_z |  =  | target_z |
    | 0     0     0  1    |     | 1        |     | 1        |
    ...
    The equation can be rewritten as:
                                                                                ...
                                                                                | t_12 |
                                                                                | t_14 |
    | source_x  source_y  1  0         0         0  0         0         0 |     | t_21 |     | target_x |
    | 0         0         0  source_x  source_y  1  0         0         0 |  *  | t_22 |  =  | target_y |
    | 0         0         0  0         0         0  source_x  source_y  1 |     | t_24 |     | target_z |
                                                                                | t_31 |
                                                                                | t_32 |
                                                                                | t_34 |

    Or in short (for the i-th point pair):  A(i) * x = b(i)

    The equation must hold for all point pairs. Thus we combine A(1) ... A(n) and
    b(1) ... b(n), respectively, to A = [A(1) ... A(n)]^T and b = [b(1) ... b(n)]^T.
    This leads to an over-determined system, which can be solved via the pseudo inverse:
    ...
    */
    ...
    A.block(1 * number_points, 3, number_points, 1) = super::m_source_points.block(0, 0, 1, number_points).transpose();
    A.block(1 * number_points, 4, number_points, 1) = super::m_source_points.block(1, 0, 1, number_points).transpose();
    A.block(1 * number_points, 5, number_points, 1) = ones;
    A.block(2 * number_points, 6, number_points, 1) = super::m_source_points.block(0, 0, 1, number_points).transpose();
    A.block(2 * number_points, 7, number_points, 1) = super::m_source_points.block(1, 0, 1, number_points).transpose();
    A.block(2 * number_points, 8, number_points, 1) = ones;
    ...
    super::m_transformation_matrix(0, 2) = 0;
    super::m_transformation_matrix(0, 3) = x(2);

    super::m_transformation_matrix(1, 0) = x(3);
    super::m_transformation_matrix(1, 1) = x(4);
    super::m_transformation_matrix(1, 2) = 0;
    super::m_transformation_matrix(1, 3) = x(5);

    super::m_transformation_matrix(2, 0) = x(6);
    super::m_transformation_matrix(2, 1) = x(7);
    super::m_transformation_matrix(2, 2) = 0;
    super::m_transformation_matrix(2, 3) = x(8);
...
template <class T>
typename EstimateAffineTransformationFromPlaneTo3D<T>::Matrix3T EstimateAffineTransformationFromPlaneTo3D<T>::getLinearTransformationMatrix() const
{
...
template <class T>
typename EstimateAffineTransformationFromPlaneTo3D<T>::Vector3T EstimateAffineTransformationFromPlaneTo3D<T>::getTranslationVector() const
{
...
#endif // ESTIMATE_AFFINE_TRANSFORMATION_FROM_PLANE_TO_3D_HPP_8970243897
