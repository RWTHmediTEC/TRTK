/*
    ...
    Version 0.1.3 (2012-10-12)
*/

/** \file EstimateSimilarityTransformation3D.hpp
  * \brief This file contains the \ref TRTK::EstimateSimilarityTransformation3D
  *        "EstimateSimilarityTransformation3D" class.
  */


#ifndef ESTIMATE_SIMILARITY_TRANSFORMATION_3D_HPP_7829730810
#define ESTIMATE_SIMILARITY_TRANSFORMATION_3D_HPP_7829730810
...
/** \class EstimateSimilarityTransformation3D
  *
  * \brief Estimates a 3D similarity transformation from two point sets.
  *
  * This class estimates a 3D similarity transformation between two point sets
  ...
  *     // in the x-y plane with a center of rotation of (1, 3, 0) and a uniform
  *     // scaling of 2.
  *
  ...
  *
  *     EstimateSimilarityTransformation3D<double> estimateSimilarityTransformation3D(source_points,
  *                                                                                   target_points);
  *     estimateSimilarityTransformation3D.compute();
  *
  *     // Display the results.
  *
  *     cout.precision(4);
  ...
  *          << estimateSimilarityTransformation3D.getTransformationMatrix() << endl << endl;
  *
  *     // Example of how to use the result.
  *
  *     Transform3D<double> transform2 = estimateSimilarityTransformation3D.getTransformationMatrix();
  *
  ...
  * Target point 1: (3.0000, 3.0000, 0.0000)
  * Target point 1: (3.0000, 3.0000, 0.0000)
  *
  * \endcode
  *
  ...
  *       this site. </a>
  *
  * \see EstimateSimilarityTransformation2D
  *
...
template <class T>
class EstimateSimilarityTransformation3D : public EstimateTransformation3D<T>
{
private:
...
public:
    ...
    EstimateSimilarityTransformation3D(const std::vector<Coordinate<T> > & source_points,
                                       const std::vector<Coordinate<T> > & target_points);

    EstimateSimilarityTransformation3D(const std::vector<Vector3T> & source_points,
                                       const std::vector<Vector3T> & target_points);

    EstimateSimilarityTransformation3D(const std::vector<Vector4T, Eigen::aligned_allocator<Vector4T> > & source_points,
...
template <class T>
EstimateSimilarityTransformation3D<T>::EstimateSimilarityTransformation3D()
{
}
...
template <class T>
EstimateSimilarityTransformation3D<T>::EstimateSimilarityTransformation3D(const std::vector<Coordinate<T> > & source_points,
                                                                          const std::vector<Coordinate<T> > & target_points) :
    scaling_factor(0)
{
...
template <class T>
EstimateSimilarityTransformation3D<T>::EstimateSimilarityTransformation3D(const std::vector<Vector3T> & source_points,
                                                                          const std::vector<Vector3T> & target_points) :
    scaling_factor(0)
{
    super::setSourcePoints(source_points);
...
template <class T>
EstimateSimilarityTransformation3D<T>::EstimateSimilarityTransformation3D(const std::vector<Vector4T, Eigen::aligned_allocator<Vector4T> > & source_points,
                                                                          const std::vector<Vector4T, Eigen::aligned_allocator<Vector4T> > & target_points) :
    scaling_factor(0)
{
    super::setSourcePoints(source_points);
...
template <class T>
void EstimateSimilarityTransformation3D<T>::compute()
{
    if (super::m_source_points.size() != super::m_target_points.size())
    {
        ErrorObj error;
        error.setClassName("EstimateSimilarityTransformation3D<T>");
    ...
    up to a possible scaling factor alpha contained in Sigma.

    To find the lost scaling factor, we use x and y again which are zero mean vectors of X and Y.
    The distance from origin (norm) of each target vector in x (column in x) is divided through the 
    distance from the origin (norm) of its corresponding source vector in y (column in y).
    The scaling factor should be the same for all target source pairs. We use the mean
    value as scaling factor and apply it to the rotation matrix:
    ...
    Note: The rotation matrix is computed via the SVD to guarantee an orthogonality.
          The scaling factor can still be computed via the almost orthogonal matrix
          [x * x^T]^(-1) * R_XY.
    ...
    */
    ...
    scaling_factor = ( Y.colwise().norm().array() / X.colwise().norm().array() ).mean();
...
/** \tparam T scalar type of the coordinates
  ...
  *
  * \see compute(), getTranslationVector(), getTransformationMatrix() and
  *      getScalingFactor()
  */

template <class T>
typename EstimateSimilarityTransformation3D<T>::Matrix3T EstimateSimilarityTransformation3D<T>::getRotationMatrix() const
{
...
/** \tparam T scalar type of the coordinates
  *
  * \return Returns the scaling as part of the sought transformation.
  *
  * \see compute(), getRotationMatrix(), getTransformationMatrix() and
  *      getTranslationVector()
  */

template <class T>
typename EstimateSimilarityTransformation3D<T>::value_type EstimateSimilarityTransformation3D<T>::getScalingFactor() const
{
...
/** \tparam T scalar type of the coordinates
  ...
  *
  * \see compute(), getTransformationMatrix(), getRotationMatrix() and
  *      getScalingFactor()
  */

template <class T>
typename EstimateSimilarityTransformation3D<T>::Vector3T EstimateSimilarityTransformation3D<T>::getTranslationVector() const
{
...
#endif // ESTIMATE_SIMILARITY_TRANSFORMATION_3D_HPP_7829730810
