/*
    Two-dimensional interpolation for irregularly-spaced data.
    ...
    Version 0.1.1 (2013-08-20)
*/

/** \file Interpolation2D.hpp
  * \brief This file contains the \ref TRTK::Interpolation2D "Interpolation2D" class.
  */

#ifndef INTERPOLATION_2D_HPP_1443431201
#define INTERPOLATION_2D_HPP_1443431201
...
/** \brief Two-dimensional interpolation for irregularly-spaced data.
  *
  ...
  *
  * This class provides a two-dimensional interpolation for irregularly-spaced
  ...
  *         // Setup data.
  *
  *         typedef Interpolation2D<double, Coordinate<double>, double> Interpolation2D;
  *         typedef Interpolation2D::Point Point;
  *
  *         std::vector<Point> data_points;
  *         std::vector<Interpolation2D::data_type> data;
  *
  *         data_points.push_back(Point(1 ,1));
  *         data.push_back(1);
  *
  *         data_points.push_back(Point(2, -1));
  *         data.push_back(2);
  *
  *         data_points.push_back(Point(0, 0));
  *         data.push_back(3);
  *
  *         data_points.push_back(Point(1, 0));
  *         data.push_back(4);
  *
  *         data_points.push_back(Point(-1, 1));
  *         data.push_back(5);
  ...
  *
  *         Interpolation2D interpolation(data_points, data, 0);
  *
  *         interpolation.setInterpolationMethod(Interpolation2D::INVERSE_DISTANCE_WEIGHTING);
  *         interpolation.setNumberOfNearestNeighbors(10);
  *         interpolation.setRadius(10);
  *
  *         // Get interpolated values.
  *
  *         std::cout << interpolation(1.2, 0.1) << std::endl;
  *     }
  ...
  * // Setup data.
  *
  * typedef Interpolation2D<double> Interpolation2D;
  * typedef Interpolation2D::Point Point;
  *
  * std::vector<Point> data_points;
  * std::vector<Interpolation2D::data_type> data;
  ...
  * // Setup interpolation class.
  *
  * Interpolation2D interpolation(data_points, data, Point(0, 0, 0));
  *
  * // Get interpolated values.
  *
  * std::cout << interpolation(1.2, 0.1) << std::endl;
  ...
  *         // Setup data.
  *
  *         typedef Interpolation2D<double, Coordinate<double>, RGB> Interpolation2D;
  *         typedef Interpolation2D::Point Point;
  *
  *         std::vector<Point> data_points;
  *         std::vector<Interpolation2D::data_type> data;
  ...
  *         // Setup interpolation class.
  *
  *         Interpolation2D interpolation(data_points, data);
  *
  *         interpolation.setInterpolationMethod(Interpolation2D::INVERSE_DISTANCE_WEIGHTING);
  *         interpolation.setRadius(10);
  *
  *         // Get interpolated values.
  *
  *         RGB value = interpolation(1.2, 0.1);
  *
  *         std::cout << "("  << value[0]
  *                   << ", " << value[1]
  *                   << ", " << value[2]
...
public:
    ...
    Interpolation2D(const DataType & default_value = DataType(),
                    InterpolationMethod interpolation_method = INVERSE_DISTANCE_WEIGHTING_KNN);

    Interpolation2D(const std::vector<Point> & data_points,
    ...
    DataType operator()(ValueType x, ValueType y) const;

    DataType getData(const Point & point) const;
    DataType getData(ValueType x, ValueType y) const;
...
protected:
    ...
    DataType (Interpolation2D::*interpolation_function)(ValueType, ValueType) const;

    DataType getDataInverseDistanceWeighting(ValueType x, ValueType y) const;
    DataType getDataInverseDistanceWeightingKNN(ValueType x, ValueType y) const;
    DataType getDataNearestNeighbor(ValueType x, ValueType y) const;
...
/** \brief Constructs an instance of Interpolation2D.
  *
...
Interpolation2D<ValueType, PointType, DataType>::Interpolation2D(const DataType & default_value, InterpolationMethod interpolation_method) :
    exponent(2),
    radius(1),
    tree(NULL),
    data_type_default_value(default_value)
{
    query_point = flann::Matrix<ValueType>(new ValueType[2], 1, 2);
...
/** \brief Constructs an instance of Interpolation2D.
  *
  * \param [in] data_points                 Positions (in 2D) of the below data.
...
template<class ValueType, class PointType, class DataType>
Interpolation2D<ValueType, PointType, DataType>::Interpolation2D(const std::vector<Point> & data_points,
                                                                 const std::vector<DataType> & data,
...
/** \brief Destructs the instance of Interpolation2D. */

template<class ValueType, class PointType, class DataType>
Interpolation2D<ValueType, PointType, DataType>::~Interpolation2D()
{
    data_points.free();
    query_point.free();
    distances.free();
...
template<class ValueType, class PointType, class DataType>
inline DataType Interpolation2D<ValueType, PointType, DataType>::operator()(const Point & point) const
{
    return (this->*interpolation_function)(point.x(), point.y());
}
...
template<class ValueType, class PointType, class DataType>
inline DataType Interpolation2D<ValueType, PointType, DataType>::operator()(ValueType x, ValueType y) const
{
    return (this->*interpolation_function)(x, y);
}
...
template<class ValueType, class PointType, class DataType>
inline DataType Interpolation2D<ValueType, PointType, DataType>::getData(const Point & point) const
{
    return (this->*interpolation_function)(point.x(), point.y());
}
...
template<class ValueType, class PointType, class DataType>
inline DataType Interpolation2D<ValueType, PointType, DataType>::getData(ValueType x, ValueType y) const
{
    return (this->*interpolation_function)(x, y);
}
...
/** \brief Performs an Inverse Distance Weighting.
  ...
  *
  * An interpolated value \f$ data' \f$ at postion \f$ (x, y) \f$ is computed
  ...
  * \f]
  * where \f$ d_i \f$ is the distance between the given point \f$ (x, y) \f$
...
template<class ValueType, class PointType, class DataType>
DataType Interpolation2D<ValueType, PointType, DataType>::getDataInverseDistanceWeighting(ValueType x, ValueType y) const
{
    if (this->tree->size() == 0)
    {
        throw std::range_error("Interpolation2D::getDataInverseDistanceWeighting(): Tree is empty.");
    }
    ...
    {
        throw std::runtime_error("Interpolation2D::getDataInverseDistanceWeighting(): Radius too small. No nearest neighbor found.");
    }
...
template<class ValueType, class PointType, class DataType>
DataType Interpolation2D<ValueType, PointType, DataType>::getDataInverseDistanceWeightingKNN(ValueType x, ValueType y) const
{
    if (this->tree->size() == 0)
    {
        throw std::range_error("Interpolation2D::getDataInverseDistanceWeightingKNN(): Tree is empty.");
    }
...
template<class ValueType, class PointType, class DataType>
DataType Interpolation2D<ValueType, PointType, DataType>::getDataNearestNeighbor(ValueType x, ValueType y) const
{
    if (this->tree->size() == 0)
    {
        throw std::range_error("Interpolation2D::getDataNearestNeighbor(): Tree is empty.");
    }
...
template<class ValueType, class PointType, class DataType>
void Interpolation2D<ValueType, PointType, DataType>::setData(const std::vector<Point> & data_points,
                                                              const std::vector<DataType> & data)
{
    ...
    this->data_points = flann::Matrix<ValueType>(new ValueType[2 * number_points], number_points, 2);;
...
template<class ValueType, class PointType, class DataType>
void Interpolation2D<ValueType, PointType, DataType>::setExponent(ValueType value)
{
    this->exponent = value;
}
...
template<class ValueType, class PointType, class DataType>
void Interpolation2D<ValueType, PointType, DataType>::setInterpolationMethod(InterpolationMethod interpolation_method)
{
    switch (interpolation_method)
    {
        case INVERSE_DISTANCE_WEIGHTING:
        {
            interpolation_function = &Interpolation2D::getDataInverseDistanceWeighting;
            break;
        }

        case INVERSE_DISTANCE_WEIGHTING_KNN:
        {
            interpolation_function = &Interpolation2D::getDataInverseDistanceWeightingKNN;
            break;
        }

        case NEAREST_NEIGHBOR:
        {
            interpolation_function = &Interpolation2D::getDataNearestNeighbor;
            break;
        }

        default:
        {
            throw std::logic_error("Interpolation2D::setInterpolationMethod(): Unknown interpolation method.");
        }
    }
}
...
template<class ValueType, class PointType, class DataType>
void Interpolation2D<ValueType, PointType, DataType>::setNumberOfNearestNeighbors(unsigned number)
{
    if (number == 0)
    {
        throw std::logic_error("Interpolation2D::setNumberOfNearestNeighbors(): "
...
template<class ValueType, class PointType, class DataType>
void Interpolation2D<ValueType, PointType, DataType>::setRadius(ValueType radius)
{
    if (radius <= 0)
    {
        throw std::logic_error("Interpolation2D::setRadius(): Radius must be greater than zero.");
    }
...
#endif // INTERPOLATION_2D_HPP_1443431201
